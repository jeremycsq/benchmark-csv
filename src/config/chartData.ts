...
export interface PageBarChartConfig {
  title: string
...
export const pageBarChartConfigs: Record<string, PageBarChartConfig> = {
  engagement: {
    title: 'Engagement',
    badgeColor: '#E5D0F6',
...
export const getPageBarChartConfig = (pageName: string): PageBarChartConfig => {
  return pageBarChartConfigs[pageName] || pageBarChartConfigs.engagement
}

// Fonction pour randomiser les données d'une page
export const randomizePageBarChartData = (pageName: string): BarChartItem[] => {
  const config = getPageBarChartConfig(pageName)

  return config.barChartData.map((item) => ({
    ...item,
    values: [
